...
namespace Telemarketing.Dal
{
    internal class DalA
    {
        private static DalA instance;
        private OleDbConnection con;
        private DataSet ds;
        private DalA(string connectionString)
        {
            con = new OleDbConnection(connectionString);
            ds = new DataSet();
        }
        public static DalA GetInstance()
        {
            if (instance == null)
            {
                string path = System.IO.Directory.GetCurrentDirectory();
                int x = path.IndexOf("\\bin");
                path = path.Substring(0, x) + "\\Dal\\TelemarketingDB.accdb";
                instance = new DalA(@"provider=Microsoft.ACE.OLEDB.12.0;Data Source='" + path + "';Persist Security Info=True");
            }
            return instance;
        }
